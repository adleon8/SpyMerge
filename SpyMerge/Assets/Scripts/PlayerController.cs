...
public class PlayerController : MonoBehaviour
{
    ...
    private void OnConnectedToServer()
    {
        ...
        keyboardAction = gameplayActionMap.FindAction("KeyboardInput");

        clickAction.performed += OnClickPerformed;
        rightClickAction.performed += OnRightClickPerformed;
        keyboardAction.performed += OnKeyboardPerformed;
        moveAction.performed += context => moveDir = context.ReadValue<Vector2>();
        moveAction.canceled += context => moveDir = Vector2.zero;
    ...
    {
        if (currentItemUI != null && currentItemUI.item != null)
        {
            currentItemUI.item.Use(gameObject);
            currentItemUI.ClickItem();
        }
    }
    ...
    {
        KeyControl key = context.control as KeyControl;
    ...
    public void HandleInput()
    {
        ...
        {
            Vector3 moveVector = new Vector3(moveDir.x, 0, moveDir.y) * speed * Time.deltaTime;
